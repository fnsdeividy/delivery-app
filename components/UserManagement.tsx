...
export function UserManagement({ storeSlug }: UserManagementProps) {
  ...
  const { data: usersData, isLoading: isLoadingUsers, error: usersError } = useUsers(currentPage, 10)
  ...
  if (usersError) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-md p-4">
        <p className="text-red-700">Erro ao carregar usuários: {usersError.message}</p>
      </div>
    )
  }
              ...
              <tr key={user.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                    ...
                    user.role === UserRole.EMPLOYEE ? 'bg-yellow-100 text-yellow-800' :
