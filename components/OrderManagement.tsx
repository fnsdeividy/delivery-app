...
export function OrderManagement({ storeSlug }: OrderManagementProps) {
  ...
  // Hooks para pedidos
  const { data: ordersData, isLoading: isLoadingOrders, error: ordersError } = useOrders(storeSlug, currentPage, 10)
  const createOrderMutation = useCreateOrder()
  const updateOrderMutation = useUpdateOrder()
  const cancelOrderMutation = useCancelOrder()
  
  // Stats dos pedidos
  const { data: orderStats, isLoading: isLoadingStats } = useOrderStats(storeSlug)
  ...
  if (ordersError) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-md p-4">
        <p className="text-red-700">Erro ao carregar pedidos: {ordersError.message}</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      {orderStats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="text-2xl font-bold text-green-600">R$ {orderStats.totalRevenue.toFixed(2)}</div>
              ...
              <tr key={order.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getPaymentStatusColor(order.paymentStatus)}`}>
                    {order.paymentStatus === PaymentStatus.PAID ? 'Pago' : 
                     order.paymentStatus === PaymentStatus.PENDING ? 'Pendente' : 
                     order.paymentStatus === PaymentStatus.FAILED ? 'Falhou' : 'N/A'}
                  </span>
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex flex-col space-y-2">
                    ...
                    {order.status === OrderStatus.READY && order.type === OrderType.DELIVERY && (
                      <button
                        onClick={() => handleStatusChange(order.id, OrderStatus.DELIVERING)}
                    ...
                    {order.status === OrderStatus.READY && order.type === OrderType.PICKUP && (
                      <button
