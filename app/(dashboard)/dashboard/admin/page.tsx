...
export default function AdminDashboard() {
  ...
  const checkAuth = useCallback(async () => {
    try {
      ...
      loadAdminData(payload.role, payload.email);
    } catch (error) {
  ...
  const loadSuperAdminData = useCallback(async () => {
    try {
      // Carregar estatísticas do sistema completo
      ...
      // Carregar todas as lojas recentes
  ...
  const loadAdminDataForUser = useCallback(async (userEmail: string) => {
    try {
      // Carregar apenas as lojas do admin logado
      const storesData = await apiClient.get<any>(`/stores?page=1&limit=10`);
      const userStores = storesData.data || [];

      // Filtrar apenas lojas criadas pelo admin (backend já faz isso, mas garantimos aqui também)
      const adminStores = userStores.filter(
        (store: any) => store.createdByEmail === userEmail
      );

      // Estatísticas apenas das lojas do admin
      setStats({
        totalStores: adminStores.length,
        activeStores: adminStores.filter((store: any) => store.approved).length,
        pendingStores: adminStores.filter((store: any) => !store.approved)
          .length,
        totalUsers: 0, // TODO: Implementar contagem de usuários das lojas
        totalOrders: 0, // TODO: Implementar contagem de pedidos das lojas
        totalRevenue: "R$ 0,00", // TODO: Implementar cálculo de receita das lojas
        systemHealth: "excellent",
      });

      // Formatar lojas do admin
      const formattedStores = adminStores.slice(0, 5).map((store: any) => ({
        id: store.id,
        name: store.name,
        slug: store.slug,
        status: store.approved ? "active" : "pending",
  ...
  const loadAdminData = useCallback(
    async (role: string, userEmail: string) => {
      try {
        if (role === "SUPER_ADMIN") {
          // SUPER_ADMIN vê todas as lojas e estatísticas gerais
          await loadSuperAdminData();
        } else {
          // ADMIN vê apenas suas próprias lojas e estatísticas
          await loadAdminDataForUser(userEmail);
        }
      } catch (error) {
        console.error("Erro ao carregar dados administrativos:", error);
      }
    },
    [loadSuperAdminData, loadAdminDataForUser]
  );
  ...
  const navigationActions = useMemo(() => {
    if (isSuperAdmin) {
      // SUPER_ADMIN vê todas as opções
      return [
        {
      ...
      // ADMIN vê apenas opções relacionadas à sua loja
      return [
        {
          href: "/dashboard",
          icon: Gear,
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">
            Você não tem permissão para acessar o Dashboard Admin.
          </p>
          <button
            ...
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {isSuperAdmin
                    ? "Painel de Controle do Sistema Completo"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">
                    {isSuperAdmin ? "Total de Lojas" : "Minhas Lojas"}
                  </p>
                  <div className="flex items-center">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">
                    {isSuperAdmin ? "Usuários" : "Funcionários"}
                  </p>
                  <div className="flex items-center">
                    <p className="text-2xl font-semibold text-gray-900">
                      {stats.totalUsers}
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">
                    {isSuperAdmin ? "Pedidos Hoje" : "Pedidos das Minhas Lojas"}
                  </p>
                  <div className="flex items-center">
                    <p className="text-2xl font-semibold text-gray-900">
                      {stats.totalOrders}
            ...
            <div className="bg-white rounded-lg shadow">
              <div className="px-6 py-4 border-b border-gray-200">
                <h3 className="text-lg font-medium text-gray-900">
                  {isSuperAdmin ? "Lojas Recentes" : "Sua Loja"}
                </h3>
              </div>
              <div className="p-6">
                        ...
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {isSuperAdmin ? `por ${store.owner}` : "Sua loja"}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                ...
                <>
                  <div className="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        Pizzaria do João foi criada por joao@email.com
                      </p>
                    </div>
                  ...
                  <div className="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        maria@email.com foi aprovada como lojista
                      </p>
                    </div>
                ...
                <>
                  <div className="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        Loja criada com sucesso e está ativa
                      </p>
                    </div>
                    <span className="text-xs text-gray-400">2 min atrás</span>
                  </div>
                  ...
                  <div className="flex items-center space-x-4 p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500">
                        Configure produtos, horários e pagamentos
                      </p>
                    </div>
                    <span className="text-xs text-gray-400">15 min atrás</span>
                  </div>
