...
describe("Filtro de Lojas por Criador", () => {
  ...
  describe("Listagem de Lojas", () => {
    ...
    it("deve retornar lojas legacy para ADMIN", async () => {
      // Arrange
      const mockStoresData = {
        data: [
          {
      ...
      // Assert
      expect(result.data).toHaveLength(2);
      expect(result.data.some((store) => store.createdByEmail === null)).toBe(
        true
      );
    });
  });
  ...
  describe("Cenários de Diferentes Roles", () => {
    ...
    it("ADMIN deve ver todas as lojas", async () => {
      // Arrange
      const allStores = {
        data: [
          {
      ...
      expect(
        result.data.some((store) => store.createdByEmail === "admin1@teste.com")
      ).toBe(true);
      expect(
        result.data.some((store) => store.createdByEmail === "admin2@teste.com")
      ).toBe(true);
      expect(result.data.some((store) => store.createdByEmail === null)).toBe(
        true
      );
