...
export function validateStoreApproval(data: StoreApprovalValidation): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (!data.userRole || data.userRole !== "admin") {
    errors.push("Usuário deve ter permissão de administrador");
  }
...
export function validateStoreRejection(data: StoreRejectionValidation): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
