...
export function useCreateProduct() {
  ...
  return useMutation({
    mutationFn: (productData: CreateProductDto) => apiClient.createProduct(productData),
    onSuccess: (_, productData) => {
      queryClient.invalidateQueries({ queryKey: ['products', productData.storeSlug] })
      queryClient.invalidateQueries({ queryKey: ['categories', productData.storeSlug] })
    },
  })
}
...
export function useCreateCategory() {
  ...
  return useMutation({
    mutationFn: (categoryData: CreateCategoryDto) => apiClient.createCategory(categoryData),
    onSuccess: (_, categoryData) => {
      queryClient.invalidateQueries({ queryKey: ['categories', categoryData.storeSlug] })
    },
  })
}
...
export function useUpdateCategory() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { id }) => {
      ...
      // Invalidar todas as categorias para atualizar a lista
...
export function useDeleteCategory() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { storeSlug }) => {
      ...
      // Também invalidar produtos que podem ter sido afetados
      queryClient.invalidateQueries({ queryKey: ['products', storeSlug] })
    },
  })
}
