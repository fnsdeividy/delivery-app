...
export function useCreateOrder() {
  ...
  return useMutation({
    mutationFn: (orderData: CreateOrderDto) => apiClient.createOrder(orderData),
    onSuccess: (_, orderData) => {
      queryClient.invalidateQueries({ queryKey: ['orders', orderData.storeSlug] })
      queryClient.invalidateQueries({ queryKey: ['orders', orderData.storeSlug, 'stats'] })
    },
  })
}
...
export function useUpdateOrder() {
  ...
  return useMutation({
    mutationFn: ({ id, orderData }: { id: string; orderData: UpdateOrderDto }) =>
      apiClient.updateOrder(id, orderData),
...
export function useOrdersByStatus(storeSlug: string, status: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await apiClient.getOrders(storeSlug, 1, 100)
      return {
        ...response,
...
export function useOrdersByPeriod(storeSlug: string, period: 'today' | 'week' | 'month') {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return {
        ...response,
        data: response.data.filter(order => new Date(order.createdAt) >= startDate)
      }
    },
